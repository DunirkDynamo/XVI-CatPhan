...
- Or run as a module: `python -m catphan_analysis.select_and_analyze`
...
# Build executables
scripts\build_executables.bat  # Windows
# For manual/non-Windows builds, run PyInstaller with:
# python -m PyInstaller packaging/pyinstaller/CatPhanAnalyzer.spec
```

---

## Running Without Console Entry Points

If you have the package installed but prefer to invoke modules directly instead of the console entry points:

```bash
# Run installed package modules directly
python -m catphan_analysis.select_and_analyze           # GUI
python -m catphan_analysis.main /path/to/dicom          # Command-line
...
- Review `docs/guides/HOW_TO_USE.md` for usage instructions
...
After installation:
- Read [docs/guides/HOW_TO_USE.md](docs/guides/HOW_TO_USE.md) for usage instructions
- See [docs/reference/QUICK_REFERENCE.md](docs/reference/QUICK_REFERENCE.md) for API reference
