...
**Without using the console entry points**, you can run the installed package modules directly:
```bash
...
See [docs/examples.rst](../examples.rst) for more detailed usage patterns.
...
5. **Use the GUI first:** Try `catphan-select` on a known-good dataset before setting up automation
...
- Check [docs/examples.rst](../examples.rst) for advanced usage patterns
